...
$MachineList = (Get-ClusterNode -Cluster s-cluster).Name
# $domain = "az31r08"
# $userpwd = "noharper"
# $username = "MASi$g8!"
...
$HostSession = New-PSSession -ComputerName $MachineList[0]

$MachineList | ForEach-Object {

    $MachineName = $_
    $DestinationSession = New-PSSession -ComputerName $MachineName

    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "mkdir C:\Test-NetStack\tools"} -ErrorAction SilentlyContinue
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "mkdir C:\Test-NetStack\tools\NDK-Perf"} -ErrorAction SilentlyContinue
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "mkdir C:\Test-NetStack\tools\NDK-Ping"} -ErrorAction SilentlyContinue
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "mkdir C:\Test-NetStack\tools\CTS-Traffic"} -ErrorAction SilentlyContinue
    ...
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "sc delete NDKPerf type=kernel binpath=C:\Test-NetStack\tools\NDK-Perf\NDKPerf.sys"}
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "sc create NDKPerf type=kernel binpath=C:\Test-NetStack\tools\NDK-Perf\NDKPerf.sys"}
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "sc delete NDKPing type=kernel binpath=C:\Test-NetStack\tools\NDK-Ping\NDKPing.sys"}
    Invoke-Command -ComputerName $MachineName -ScriptBlock {cmd /c "sc create NDKPing type=kernel binpath=C:\Test-NetStack\tools\NDK-Ping\NDKPing.sys"}
