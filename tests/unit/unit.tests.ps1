...
Describe "$($env:APPVEYOR_BUILD_FOLDER)-Manifest" {
    Context Validation {
        ...
        'icmp.psm1', 'internal.psm1', 'ndk.psm1', 'tcp.psm1' | ForEach-Object {
            ...
            It "[Test-Path] - $($env:repoName)\$thisModule.psm1 exists" { Test-Path "$($env:repoName)\$thisModule.psm1" | Should Be True }

            Import-Module .\$($env:repoName)\$thisModule.psm1 -Force
            $Module = Get-Module $thisModule

            It "[Import-Module] - $($env:repoName)\$thisModule.psm1 is a valid PowerShell Module" {
